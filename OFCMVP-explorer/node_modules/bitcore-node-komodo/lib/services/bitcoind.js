...
Bitcoin.prototype.getAddressBalance = function(addressArg, options, callback) {
  ...
  if (balance) {
    ...
    this.client.getAddressBalance({addresses: addresses}, function(err, response) {
      ...
      // try to query for cc address balance
      if (!response.result.balance) {
        self.client.getAddressBalance({addresses: addresses}, true, function(err, response) {
          if (err) {
            return callback(self._wrapRPCError(err));
          }
  ...
  function finish(mempoolDeltas) {
    if (utxos) {
      ...
      self.client.getAddressUtxos({addresses: addresses}, function(err, response) {
        ...
        // try to query for cc address utxos
        if (!utxos.length) {
  ...
  function finish() {
    if (txids && !rangeQuery) {
      ...
      self.client.getAddressTxids(txidOpts, function(err, response) {
        ...
        // try to query for cc address txids
        if (!allTxids.length) {
...
Bitcoin.prototype._getAddressDetailedTransaction = function(txid, options, next) {
  ...
  self.getDetailedTransaction(
    ...
    function(err, transaction) {
      ...
      if (options.fromRaw) {
        self.getRawTransaction(txid, function(err, rawtransaction) {
          if (err) {
            return next(err);
          }
          
          self.node.services.bitcoind.decodeRawTransaction(rawtransaction.toString('hex'), function(err, decodedtx) {            
            if (err) {
              return next(err);
            }

            for (var i = 0; i < details.tx.outputs.length; i++) {
              if (decodedtx.vout[i].scriptPubKey && decodedtx.vout[i].scriptPubKey.condition) {
                details.tx.outputs[i].condition = decodedtx.vout[i].scriptPubKey.condition;
              }
            }
...
Bitcoin.prototype.oraclesaddress = function(pubkey, callback) {
  var self = this;
  this.client.oraclesaddress(pubkey, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var result = response.result;
...
Bitcoin.prototype.oracleslist = function(callback) {
  var self = this;
  this.client.oracleslist(function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var result = response.result;
...
Bitcoin.prototype.oraclesinfo = function(txid, callback) {
  var self = this;
  this.client.oraclesinfo(txid, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var result = response.result;
...
Bitcoin.prototype.oraclessamples = function(txid, address, num, callback) {
  var self = this;
  this.client.oraclessamples(txid, address, num, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var result = response.result;
...
Bitcoin.prototype.oraclessample = function(otxid, txid, callback) {
  var self = this;
  this.client.oraclessample(otxid, txid, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var result = response.result;
...
Bitcoin.prototype.getRawMemPool = function(callback) {
  var self = this;
...
Bitcoin.prototype.decodeRawTransaction = function(rawtx, callback) {
  var self = this;
  this.client.decodeRawTransaction(rawtx, function(err, response) {
    if (err) {
      return callback(self._wrapRPCError(err));
    }
    var result = response.result;
