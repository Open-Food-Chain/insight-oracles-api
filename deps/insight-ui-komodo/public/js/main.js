...
.controller('CurrencyController',
  function($scope, $rootScope, Currency) {
    ...
    // Get initial value
    Currency.get({}, function(res) {
      $rootScope.currency.factor = $rootScope.currency.bitstamp = res.data && res.data.bitstamp;
    });
  });
  ...
  function($scope, $rootScope, $http, Global, getSocket, BlocksIndex) {
    ...
    var _getNota = function() {
      if (_explorers[$rootScope.coin].notarized) {
        $http.get('https://' + $rootScope.coin + '.explorer.dexstats.info/insight-api-komodo/status?q=getInfo')
...
.controller('ScannerController',
  function($scope, $rootScope, $modalInstance, Global) {
    ...
    var cameraInput, video, canvas, $video, context, localMediaStream;
...
angular
.module('insight.transactions')
.controller('SendRawTransactionController',
  function($scope, $http) {
    $scope.transaction = '';
...
.directive('scroll', function($window) {
  return function(scope, element, attrs) {
    ...
    .bind('scroll', function() {
      scope.secondaryNavbar = this.pageYOffset >= 200 ? true : false;
      scope.$apply();
    });
  };
})
  ...
  return {
    ...
    link: function(scope, elm, attr) {
      ...
      var handler = function() {
        ...
        var elemBottom = elm[0].offsetTop + elm[0].offsetHeight;
...
.directive('clipCopy', function() {
  ...
  return {
    ...
    template: '<div class="tooltip fade right in">' +
